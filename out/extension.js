...
var Logger = class _Logger {
  ...
  static getInstance(channel) {
    if (!_Logger.instance) {
      _Logger.instance = new _Logger(channel);
    }
...
// src/constants.ts
var OUTPUT_CHANNEL_NAME = "XTECH Supabase";
var VIEW_ID = "xtech-supabase.supabaseExplorer";
var CONFIG_PROJECT_PATH = "xtech-supabase.projectPath";
var CONFIG_AUTH_MODE = "xtech-supabase.authMode";
var CONFIG_REFRESH_INTERVAL = "xtech-supabase.refreshInterval";
var CONFIG_LOCAL_DB_URL = "xtech-supabase.localDbUrl";
var CONFIG_LINKED_PROJECT_REF = "xtech-supabase.linkedProjectRef";
var SECRET_TOKEN_KEY = "xtech-supabase.token";
...
function getSettings() {
  ...
  return {
    projectPath: config.get(CONFIG_PROJECT_PATH),
    authMode: config.get(CONFIG_AUTH_MODE, "cli"),
    refreshInterval: config.get(CONFIG_REFRESH_INTERVAL, 0),
    localDbUrl: config.get(CONFIG_LOCAL_DB_URL),
    linkedProjectRef: config.get(CONFIG_LINKED_PROJECT_REF)
  };
}

// src/projectDiscovery.ts
var SupabaseProjectDiscovery = class {
...
var AuthProvider = class {
  ...
  async getCliToken() {
    try {
      ...
      if (home) {
        const tokenPaths = [
          path2.join(home, ".supabase", "access-token"),
          path2.join(home, ".config", "supabase", "access-token")
        ];
        for (const tokenPath of tokenPaths) {
          if (fs2.existsSync(tokenPath)) {
            const fileToken = this.normalizeToken(
              fs2.readFileSync(tokenPath, "utf-8")
            );
            if (fileToken) {
              return fileToken;
            }
  ...
  async getLinkedProjectRef(projectRoot) {
    const settings = getSettings();
  ...
  getLinkedProjectRefFromConfig(projectRoot) {
    try {
      const configPath = path2.join(projectRoot, "supabase", "config.toml");
...
var LocalEnvironment = class {
  ...
  async getDbUrl() {
    ...
    try {
      const configPath = path3.join(this.projectRoot, "supabase", "config.toml");
...
// src/environment/linkedEnvironment.ts
var child_process3 = __toESM(require("child_process"));
var LinkedEnvironment = class {
  ...
  execCommand(cmd, args) {
    return new Promise((resolve, reject) => {
      child_process3.execFile(cmd, args, { timeout: 3e4 }, (err, stdout, stderr) => {
        if (err) {
...
// src/migrations/migrationService.ts
var fs4 = __toESM(require("fs"));
var path4 = __toESM(require("path"));
var MIGRATION_NAME_RE = /^[a-zA-Z0-9_]+$/;
var MigrationService = class _MigrationService {
  ...
  async listMigrations() {
    const migrationsDir = path4.join(this.projectRoot, "supabase", "migrations");
    let sqlFiles = [];
    try {
      if (fs4.existsSync(migrationsDir)) {
        sqlFiles = fs4.readdirSync(migrationsDir).filter((f) => f.endsWith(".sql")).sort();
      }
    } catch (err) {
    ...
    return sqlFiles.map((file) => {
      ...
      return {
        name,
        filePath: path4.join(migrationsDir, file),
        timestamp,
        status: _MigrationService.computeStatus(isLocal, isLinked),
  ...
  async createMigration(name) {
    ...
    fs4.writeFileSync(filePath, `-- Migration: ${trimmed}
...
// src/database/introspectionQueries.ts
var QUERY_SCHEMAS = `
SELECT schema_name
...
var DatabaseService = class {
  ...
  async query(sql) {
    if (this.canUsePostgres()) {
      return this.queryViaPostgres(sql);
    }
    if (this.token) {
  ...
  canUsePostgres() {
    return this.dbUrl.startsWith("postgres://") || this.dbUrl.startsWith("postgresql://");
  }
  async queryViaPostgres(sql) {
  ...
  async getSchemas() {
    try {
      return await this.query(QUERY_SCHEMAS);
    } catch (err) {
  ...
  async getTriggers() {
    try {
      return await this.query(QUERY_TRIGGERS);
    } catch (err) {
  ...
  async getEnums() {
    try {
      return await this.query(QUERY_ENUMS);
    } catch (err) {
  ...
  async getIndexes() {
    try {
      return await this.query(QUERY_INDEXES);
    } catch (err) {
  ...
  async getRoles() {
    try {
      return await this.query(QUERY_ROLES);
    } catch (err) {
  ...
  async getPolicies() {
    try {
      return await this.query(QUERY_POLICIES);
    } catch (err) {
  ...
  async getStorageBuckets() {
    try {
      return await this.query(QUERY_STORAGE_BUCKETS);
    } catch (err) {
...
// src/treeView/supabaseTreeProvider.ts
var vscode3 = __toESM(require("vscode"));

// src/treeView/treeNodes.ts
var vscode2 = __toESM(require("vscode"));
var SupabaseRootNode = class extends vscode2.TreeItem {
  constructor(label, connected) {
    ...
    this.iconPath = new vscode2.ThemeIcon(
      ...
      connected ? new vscode2.ThemeColor("testing.iconPassed") : new vscode2.ThemeColor("testing.iconFailed")
    );
  }
};
var CategoryNode = class _CategoryNode extends vscode2.TreeItem {
  constructor(label, contextValue, env) {
    ...
    this.iconPath = _CategoryNode.iconForCategory(contextValue);
  }
  static iconForCategory(contextValue) {
    ...
    return new vscode2.ThemeIcon(map[contextValue] ?? "symbol-misc");
  }
};
var LoadingNode = class extends vscode2.TreeItem {
  constructor() {
    super("Loading\u2026", vscode2.TreeItemCollapsibleState.None);
    this.iconPath = new vscode2.ThemeIcon("loading~spin");
    this.contextValue = "loading";
  }
};
var EmptyNode = class extends vscode2.TreeItem {
  constructor(message = "No items") {
    super(message, vscode2.TreeItemCollapsibleState.None);
    this.iconPath = new vscode2.ThemeIcon("info");
    this.contextValue = "empty";
  }
};
var MigrationNode = class _MigrationNode extends vscode2.TreeItem {
  constructor(migration) {
    super(migration.name, vscode2.TreeItemCollapsibleState.None);
    this.migration = migration;
    this.contextValue = "migration";
    this.description = _MigrationNode.statusLabel(migration);
    this.tooltip = `${migration.name}
Status: ${migration.status}
File: ${migration.filePath}`;
    this.iconPath = new vscode2.ThemeIcon(
      _MigrationNode.iconForStatus(migration.status)
    );
    this.command = {
      command: "vscode.open",
      title: "Open Migration",
      arguments: [vscode2.Uri.file(migration.filePath)]
    };
  }
...
var SchemaNode = class extends vscode2.TreeItem {
  constructor(schemaName, env) {
    super(schemaName, vscode2.TreeItemCollapsibleState.Collapsed);
...
var SchemaObjectGroupNode = class extends vscode2.TreeItem {
  constructor(group, schemaName, env) {
    super(
...
var TableNode = class extends vscode2.TreeItem {
  constructor(tableName, schema, env) {
    super(tableName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("table");
  }
};
var ViewNode = class extends vscode2.TreeItem {
  constructor(viewName, schema, env) {
    super(viewName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("symbol-class");
  }
};
var FunctionNode = class extends vscode2.TreeItem {
  constructor(functionName, schema, returnType, env) {
    super(functionName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("symbol-method");
  }
};
var TriggerNode = class extends vscode2.TreeItem {
  constructor(triggerName, tableName, event, env) {
    super(triggerName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("zap");
  }
};
var EnumNode = class extends vscode2.TreeItem {
  constructor(typeName, schema, values, env) {
    super(typeName, vscode2.TreeItemCollapsibleState.None);
...
Values: ${values.join(", ")} (${env})`;
    this.iconPath = new vscode2.ThemeIcon("symbol-enum");
  }
};
var IndexNode = class extends vscode2.TreeItem {
  constructor(indexName, tableName, schema, env) {
    super(indexName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("key");
  }
};
var RoleNode = class extends vscode2.TreeItem {
  constructor(roleName, env) {
    super(roleName, vscode2.TreeItemCollapsibleState.None);
...
var PolicyNode = class extends vscode2.TreeItem {
  constructor(policyName, tableName, schema, env) {
    super(policyName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("shield");
  }
};
var StorageBucketNode = class extends vscode2.TreeItem {
  constructor(bucketId, bucketName, isPublic, env) {
    super(bucketName, vscode2.TreeItemCollapsibleState.None);
    ...
    this.iconPath = new vscode2.ThemeIcon("archive");
  }
};

// src/treeView/supabaseTreeProvider.ts
var SupabaseTreeProvider = class {
  ...
  async getSchemaObjectGroupChildren(node) {
    ...
    if (node.group === "tables") {
      ...
      return tables.length > 0 ? tables.map((t) => new TableNode(t.table_name, t.schema, node.env)) : [new EmptyNode()];
    }
    const views = await db.getViews(node.schemaName);
    return views.length > 0 ? views.map((v) => new ViewNode(v.view_name, v.schema, node.env)) : [new EmptyNode()];
  }
  ...
  async fetchCategoryItems(node) {
    ...
    switch (cat) {
      ...
      case "functions": {
        ...
        return fns.map(
          (f) => new FunctionNode(f.function_name, f.schema, f.return_type, env)
        );
      }
      case "triggers": {
        if (!db) {
...
// src/commands/createMigration.ts
  ...
  await vscode4.window.withProgress(
    ...
    async () => {
      try {
        ...
        vscode4.window.showErrorMessage(`Failed to create migration: ${msg}`);
      }
    }
  );
}
...
async function activate(context) {
  const channel = vscode5.window.createOutputChannel(OUTPUT_CHANNEL_NAME);
  ...
  const treeView = vscode5.window.createTreeView(VIEW_ID, {
  ...
  async function reinitialize() {
    ...
    const projectRoot = await discovery.discover(
      vscode5.workspace.workspaceFolders
    );
    if (!projectRoot) {
      logger.info(
        "No Supabase project found (no supabase/config.toml). Tree view will be empty."
      );
      migrationService = void 0;
      treeProvider.setServices(void 0, void 0, void 0, false, false);
      treeProvider.refresh();
      return;
    }
    ...
    const linkedEnv = linkedRef ? new LinkedEnvironment(linkedRef, token, logger) : void 0;
    const localDbUrl = await localEnv.getDbUrl();
    const localDb = localRunning && localDbUrl ? new DatabaseService(localDbUrl, void 0, logger, projectRoot) : void 0;
    const linkedDb = linkedRef && token ? new DatabaseService(
    ...
    if (settings.refreshInterval > 0) {
      refreshTimer = setInterval(() => {
        ...
        treeProvider.refresh();
      }, settings.refreshInterval * 1e3);
    }
  }
  context.subscriptions.push(
    vscode5.commands.registerCommand("xtech-supabase.refresh" /* Refresh */, async () => {
    ...
    vscode5.commands.registerCommand("xtech-supabase.createMigration" /* CreateMigration */, async () => {
      if (!migrationService) {
    ...
    vscode5.commands.registerCommand("xtech-supabase.setToken" /* SetToken */, async () => {
      const t = await vscode5.window.showInputBox({
      ...
      if (t) {
        ...
        vscode5.window.showInformationMessage("Supabase access token saved.");
      }
    }),
    vscode5.commands.registerCommand("xtech-supabase.clearToken" /* ClearToken */, async () => {
      await authProvider.clearToken();
      vscode5.window.showInformationMessage("Supabase access token cleared.");
    })
  );
  context.subscriptions.push(
